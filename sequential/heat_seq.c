...
int main() {
    ...
    inicializar(matriz_atual);
    inicializar(nova_matriz);
    ...
    printf("=== Baseline Sequencial ===\n");
    printf("Matriz: %d x %d | Iterações: %d\n", N, N, MAX_ITER);
    printf("Tempo de Execução: %.6f segundos\n", tempo_total);
    printf("Validação (Centro da Placa [%d][%d]): %.2f °C\n", N/2, N/2, matriz_atual[N/2][N/2]);

    liberar_matriz(matriz_atual);
    liberar_matriz(nova_matriz);
    return 0;
}
